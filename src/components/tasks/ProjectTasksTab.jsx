...
export default function ProjectTasksTab() {
  ...
  const { data: phases } = useQuery({ queryKey: ['phases'], queryFn: () => base44.entities.Phase.list() });
  ...
  const createMutation = useMutation({
    mutationFn: (data) => base44.entities.Task.create({ ...data, task_type: 'project' }),
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: async (data, id) => {
      ...
      queryClient.invalidateQueries(['tasks']);
      if (taskPhaseId) setTimeout(() => syncPhaseStatus(taskPhaseId), 500);
    }
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex flex-wrap gap-4 items-center">
        ...
        <select className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 text-sm text-slate-800 focus:border-emerald-500 outline-none" value={filters.project_id} onChange={e => setFilters({...filters, project_id: e.target.value, phase_id: ''})}>
      ...
      {isEditing && (
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
            <h2 className="text-xl font-bold text-slate-800 mb-4">{isEditing === 'new' ? 'משימת פרויקט חדשה' : 'עריכת משימה'}</h2>
            <div className="space-y-4">
              <div>
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {filteredTasks.map(task => (
                <tr key={task.id} className={`hover:bg-slate-50/80 transition-colors group ${task.is_completed ? 'opacity-60' : ''}`}>
                  ...
                  <td className="px-6 py-4 text-sm text-slate-500">{projects?.find(p => p.id === task.project_id)?.name || '-'}</td>
              ...
              {filteredTasks.length === 0 && (
                <tr>
                  <td colSpan="7" className="p-8 text-center text-slate-500">לא נמצאו משימות פרויקט.</td>
                </tr>
              )}
            </tbody>
          </table>
